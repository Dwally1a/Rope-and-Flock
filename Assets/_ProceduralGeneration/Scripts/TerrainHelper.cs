...
public class TerrainHelper : MonoBehaviour {
	...
	public void GenerateTerrain() {
		...
		for (int x = 0; x < width; ++x)
		{
			...
			{
				//terrainHeights[x, z] = NoiseAmplitude * UnityEngine.Random.Range(0f, 1f);

				terrainHeights[x, z] = NoiseAmplitude * (Mathf.Sin(Mathf.PI * XNoiseScale * x / width) + 
														 Mathf.Sin(Mathf.PI * ZNoiseScale * z / height));
				
				// apply noise
				terrainHeights[x, z] += NoiseAmplitude * Mathf.PerlinNoise(Time.time + (XNoiseScale * x / width), 
				 														   Time.time + (ZNoiseScale * z / height)) - (NoiseAmplitude * 0.5f);
			}
		}
	...
	public void GenerateMesh() {
		...
		// update the mesh
		OutputMeshFilter.mesh = new Mesh();
